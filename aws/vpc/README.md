...
This is an opinionated Terraform module which creates VPC resources on AWS. It can be used to create a VPC with subnets for public, private, and isolated workloads.  It also creates additional resources like network ACLs.
It can be used to create a standard, stand-alone VPC or one that is attached to a transit gateway. This module is based loosely terraform-aws-modules/terraform-aws-vpc.

## Availability Zone IDs vs Names

**We strongly recommend using Availability Zone IDs (e.g., `use1-az1`, `use1-az2`) instead of Availability Zone names (e.g., `us-east-1a`, `us-east-1b`) for the following reasons:**

- **Consistency**: Zone IDs map to the same physical zone across all AWS accounts globally, while zone names can map to different physical zones in different accounts. Using zone IDs ensures the fastest performance if/when resources need to communicate across acounts.
- **Portability**: Infrastructure code using Zone IDs will work consistently when deployed to different AWS accounts, even across different organizations.
- **Reliability**: Avoids unexpected behavior when zone names differ between accounts.

The module accepts both formats in the `az` field and `azs` parameter, but Zone IDs are the recommended approach for infrastructure as code. When using Zone IDs, ensure you use them consistently in both the `azs` parameter and the `az` field of your subnet maps.

## Usage for stand-alone VPC

```hcl
module "vpc" {
  source = "github.com/TangoGroup/aws/vpc"
  ...
  public_subnets                = {
    "10.11.3.0/24"   = { cidr = "10.11.3.0/24", az = "use1-az1" }
    "10.11.4.0/24"   = { cidr = "10.11.4.0/24", az = "use1-az2" }
    "10.11.5.0/24"   = { cidr = "10.11.5.0/24", az = "use1-az3" }
  }
...
It is possible to integrate this VPC module with a transit gateway to route local traffic and/or internet traffic.

### Usage for VPC attached to hub & spoke with transit gateway, sending outbound traffic through the transit gateway.
...
module "vpc" {
  ...
  azs                           = ["use1-az1", "use1-az2", "use1-az3"]
  private_subnets  = {
    "10.11.0.0/24"   = { cidr = "10.11.0.0/24", az = "use1-az1" }
    "10.11.1.0/24"   = { cidr = "10.11.1.0/24", az = "use1-az2" }
    "10.11.2.0/24"   = { cidr = "10.11.2.0/24", az = "use1-az3" }
  }
  
  #If public subnets are needed for NAT gateways, WAFs, or external network load balancers, otherwise use '= []'
  public_subnets                = {
    "10.11.3.0/24"   = { cidr = "10.11.3.0/24", az = "use1-az1" }
    "10.11.4.0/24"   = { cidr = "10.11.4.0/24", az = "use1-az2" }
    "10.11.5.0/24"   = { cidr = "10.11.5.0/24", az = "use1-az3" }
  }
  transit_gw_attachment_subnets = ["10.0.7.208/28", "10.0.7.224/28", "10.0.7.240/28"]

  #Send internet traffic to transit gateway, so don't create igw here.
  create_igw         = false
...
Note that in the example we allocate 3 IPs because we will be provisioning 3 NAT Gateways (due to `one_nat_gateway_per_az = true` and having 3 subnets).
If, on the other hand, `one_nat_gateway_per_az = false`, or not specified, then `aws_eip.nat` would only need to allocate 1 IP.
...
- Zero NAT Gateways are created. (default behavior)
  - `enable_nat_gateway = false`
- Single NAT Gateway (default behavior if `enable_nat_gateway = true`.)
  - `enable_nat_gateway = true`
- One NAT Gateway per availability zone (best if cross AZ to NAT GW traffic costs are epected to exceed the cost of additional NAT Gateways.)
  - `enable_nat_gateway = true`
  - `one_nat_gateway_per_az = true`

### Zero NAT Gateways

This would be used for 1) having no nat gateways or 2) sending all traffic through a central transit gateway in a hub & spoke architecture.

### Single NAT Gateway

When enable_nat_gatway is activated, the module will determine create a single NAT gateway in the first public subnet of the VPC.

### One NAT Gateway per availability zone

If `one_nat_gateway_per_az = true`, then the module will place one NAT gateway in each availability zone you specify in `var.azs`. There are some requirements around using this feature flag:

- The variable `var.azs` **must** be specified.
- The number of public subnet CIDR blocks specified in `public_subnets` **must** be greater than or equal to the number of availability zones specified in `var.azs`. This is to ensure that each NAT Gateway has a dedicated public subnet to deploy to.
- the `az` key must be provided as part of the var.public_subnets variable, specificing which availability zone each subnet should be placed into.

## Subnet Types

This module supports several subnet types, each designed for specific use cases:

### Public Subnets

Public subnets are for publicly accessible resources such as load balancers. Resources in public subnets have direct internet access via an Internet Gateway and can receive traffic from the internet.

### Private Subnets

Private subnets are for most workloads such as EC2 instances. Resources in private subnets can access the internet through a NAT Gateway for outbound traffic, but cannot receive direct inbound traffic from the internet. This provides a balance between security and functionality for most application workloads.

### Database and ElastiCache Subnets

Database and ElastiCache subnets are provided for backwards compatibility. These subnet types create dedicated subnet groups for RDS and ElastiCache services respectively. For new deployments, you can use private or isolated subnets for these workloads instead.

### Isolated Subnets

Isolated subnets are for workloads that must be isolated from all non-local VPC traffic and have no direct access to the internet. Resources in isolated subnets have no routes to NAT Gateways or Internet Gateways, making them isolated from Internet access while still being able to communicate with other resources within the same VPC. Private routes to VPN or peering connections can still be made. This is useful for databases such as RDS, sensitive workloads, data processing, or services that only need to communicate with other VPC-local resources or VPC endpoints.
...
If the organization requires a permissions boundary to be attached to the VPC Flow Log role, make sure that you specify an ARN of the permissions boundary policy as `vpc_flow_log_permissions_boundary` argument. Read more about required [IAM policy for publishing flow logs](https://docs.aws.amazon.com/vpc/latest/userguide/flow-logs-cwl.html#flow-logs-iam).
...
**Important**: The VPC CIDR must be explicitly specified and should be selected from a documented, non-overlapping IP range per customer environment. This ensures proper IP address space management and prevents conflicts between VPCs and with on-premises networks or other cloud environments.

When selecting a VPC CIDR:
- Consult your organization's IP address allocation documentation
- Ensure the CIDR does not overlap with existing VPCs, on-premises networks, or other cloud environments
- Use appropriate subnet sizing to accommodate current and future resource requirements
- Document the CIDR allocation in your IP address management system
...
| [aws_network_acl_rule.isolated_inbound](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/network_acl_rule) | resource |
| [aws_network_acl_rule.isolated_outbound](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/network_acl_rule) | resource |
...
|------|-------------|------|---------|:--------:|
| <a name="input_project_name"></a> [project\_name](#input\_project\_name) | Name of the project or purpose of this VPC.  Used for naming resources in combination with project_env. | `string` | `""` | no |
| <a name="input_project_env"></a> [project\_env](#input\_project\_env) | Tier of the project or purpose of this VPC.  Used for naming resources in combination with project_name. Example: Prod, Test, Dev | `string` | `"Prod"` | no |
...
| <a name="input_azs"></a> [azs](#input\_azs) | A list of availability zones names or ids in the region. **Recommended**: Use Zone IDs (e.g., `use1-az1`) instead of zone names (e.g., `us-east-1a`) for consistency across AWS accounts. When using Zone IDs, ensure you use them consistently in both `azs` and the `az` field of subnet maps. | `list(string)` | `[]` | no |
| <a name="input_cidr"></a> [cidr](#input\_cidr) | The IPv4 CIDR block for the VPC. Must be selected from a documented, non-overlapping IP range per customer environment. | `string` | `"0.0.0.0/0"` | no |
...
| <a name="input_database_subnets"></a> [database\_subnets](#input\_database\_subnets) | A map of maps contianing database subnets inside the VPC. Expected values are are cidr and az. Valid values also include name, and tags. Name overrides the default subnet naming. | `map(any)` | `{}` | no |
...
| <a name="input_elasticache_subnets"></a> [elasticache\_subnets](#input\_elasticache\_subnets) | A map of maps contianing elasticache subnets inside the VPC. Expected values are are cidr and az. Valid values also include name, and tags. Name overrides the default subnet naming. | `map(any)` | `{}` | no |
...
| <a name="input_enable_transit_gateway"></a> [enable\_transit\_gateway](#input\_enable\_transit\_gateway) | Should be true if you want to route traffic to a transit gateway.  This module does not create the transit gateway. | `bool` | `false` | no |
...
| <a name="input_isolated_dedicated_network_acl"></a> [isolated\_dedicated\_network\_acl](#input\_isolated\_dedicated\_network\_acl) | Whether to use dedicated network ACL (not default) and custom rules for isolated subnets | `bool` | `false` | no |
| <a name="input_isolated_inbound_acl_rules"></a> [isolated\_inbound\_acl\_rules](#input\_isolated\_inbound\_acl\_rules) | Isolated subnets inbound network ACLs | `list(map(string))` | <pre>[<br>  {<br>    "cidr_block": "0.0.0.0/0",<br>    "from_port": 0,<br>    "protocol": "-1",<br>    "rule_action": "allow",<br>    "rule_number": 100,<br>    "to_port": 0<br>  }<br>]</pre> | no |
| <a name="input_isolated_outbound_acl_rules"></a> [isolated\_outbound\_acl\_rules](#input\_isolated\_outbound\_acl\_rules) | Isolated subnets outbound network ACLs | `list(map(string))` | <pre>[<br>  {<br>    "cidr_block": "0.0.0.0/0",<br>    "from_port": 0,<br>    "protocol": "-1",<br>    "rule_action": "allow",<br>    "rule_number": 100,<br>    "to_port": 0<br>  }<br>]</pre> | no |
...
| <a name="input_isolated_subnet_assign_ipv6_address_on_creation"></a> [isolated\_subnet\_assign\_ipv6\_address\_on\_creation](#input\_isolated\_subnet\_assign\_ipv6\_address\_on\_creation) | Assign IPv6 address on isolated subnet, must be disabled to change IPv6 CIDRs. This is the IPv6 equivalent of map\_public\_ip\_on\_launch | `bool` | `null` | no |
| <a name="input_isolated_subnet_ipv6_prefixes"></a> [isolated\_subnet\_ipv6\_prefixes](#input\_isolated\_subnet\_ipv6\_prefixes) | Assigns IPv6 isolated subnet id based on the Amazon provided /56 prefix base 10 integer (0-256). Must be of equal length to the corresponding IPv4 subnet list | `list(string)` | `[]` | no |
| <a name="input_isolated_subnet_suffix"></a> [isolated\_subnet\_suffix](#input\_isolated\_subnet\_suffix) | Suffix to append to isolated subnets name | `string` | `"isolated"` | no |
...
| <a name="input_isolated_subnets"></a> [isolated\_subnets](#input\_isolated\_subnets) | A map of maps contianing isolated subnets inside the VPC. Expected values are are cidr and az. Valid values also include name, and tags. Name overrides the default subnet naming. | `map(any)` | `{}` | no |
| <a name="input_ipv6_cidr"></a> [ipv6\_cidr](#input\_ipv6\_cidr) | (Optional) IPv6 CIDR block for the VPC. | `string` | `null` | no |
| <a name="input_kubernetes_subnet_suffix"></a> [intra\_kubernetes_\_suffix](#input\_kubernetes_\_subnet\_suffix) | Suffix to append to Kubernetes subnets name | `string` | `"k8s"` | no |
| <a name="input_kubernetes_subnets"></a> [kubernetes\_subnets](#input\_kubernetes\_subnets) | A map of of maps of kubernetes subnets inside the VPC. | `map(any)` | `{}` | no |
...
| <a name="input_name"></a> [name](#input\_name) | Name to be used on all the resources as identifier. Overrides project_name & project_env. | `string` | `""` | no |
...
| <a name="input_private_subnets"></a> [private\_subnets](#input\_private\_subnets) | A map of hashes contianing private subnets inside the VPC. Must include cidr, name, tags, az, and routes in hash. Name, tags, and routes may be set to `null`. Routes allows a block or routes to be specified.  Routes overrides the default `0.0.0.0/0` route that is typically created. | `map(any)` | `{}` | no |
...
| <a name="input_public_subnets"></a> [public\_subnets](#input\_public\_subnets) | A map of of maps of public subnets inside the VPC. Public subnets are used for NAT gateways, WAFs, and other public resources. | `map(any)` | `{}` | no |
...
| <a name="input_redshift_subnets"></a> [redshift\_subnets](#input\_redshift\_subnets) | A map of maps contianing redshift subnets inside the VPC. Expected values are are cidr and az. Valid values also include name, and tags. Name overrides the default subnet naming. | `map(any)` | `{}` | no |
| <a name="input_reuse_nat_ips"></a> [reuse\_nat\_ips](#input\_reuse\_nat\_ips) | Should be true if you don't want EIPs to be created for your NAT Gateways and will instead pass them in via the 'external\_nat\_ip\_ids' variable | `bool` | `false` | no |
| <a name="input_secondary_cidr_blocks"></a> [secondary\_cidr\_blocks](#input\_secondary\_cidr\_blocks) | List of secondary CIDR blocks to associate with the VPC to extend the IP Address pool | `list(string)` | `[]` | no |
| <a name="input_tags"></a> [tags](#input\_tags) | A map of tags to add to all resources | `map(string)` | `{}` | no |
| <a name="input_transit_gateway_attachment_subnets"></a> [transit\_gateway\_attachment\_subnets](#transit\_gateway\_attachment\_subnets) | A map of maps contianing transit gateway subnets inside the VPC. | `map(any)` | `{}` | no |
| <a name="input_transit_gateway_destination_cidr_block"></a> [transit\_gateway\_destination\_cidr\_blocks](#transit\_gateway\_destination\_cidr\_blocks) | Used to pass a custom destination route for transit gateway routes. If not specified, the default 0.0.0.0/0 is used as a destination route. | `string` | `0.0.0.0/0` | no |
| <a name="input_transit_gateway_id"></a> [transit\_gateway\_id](#transit\_gateway\_id) | Used to pass the id of a transit gateway to route traffic to. | `string` | `null` | no |
...
| <a name="output_isolated_network_acl_arn"></a> [isolated\_network\_acl\_arn](#output\_isolated\_network\_acl\_arn) | ARN of the isolated network ACL |
| <a name="output_isolated_network_acl_id"></a> [isolated\_network\_acl\_id](#output\_isolated\_network\_acl\_id) | ID of the isolated network ACL |
| <a name="output_isolated_route_table_association_ids"></a> [isolated\_route\_table\_association\_ids](#output\_isolated\_route\_table\_association\_ids) | List of IDs of the isolated route table association |
| <a name="output_isolated_route_table_ids"></a> [isolated\_route\_table\_ids](#output\_isolated\_route\_table\_ids) | List of IDs of isolated route tables |
| <a name="output_isolated_subnet_arns"></a> [isolated\_subnet\_arns](#output\_isolated\_subnet\_arns) | List of ARNs of isolated subnets |
| <a name="output_isolated_subnets"></a> [isolated\_subnets](#output\_isolated\_subnets) | List of IDs of isolated subnets |
| <a name="output_isolated_subnets_cidr_blocks"></a> [isolated\_subnets\_cidr\_blocks](#output\_isolated\_subnets\_cidr\_blocks) | List of cidr\_blocks of isolated subnets |
| <a name="output_isolated_subnets_ipv6_cidr_blocks"></a> [isolated\_subnets\_ipv6\_cidr\_blocks](#output\_isolated\_subnets\_ipv6\_cidr\_blocks) | List of IPv6 cidr\_blocks of isolated subnets in an IPv6 enabled VPC |
